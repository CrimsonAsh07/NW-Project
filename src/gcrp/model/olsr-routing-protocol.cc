...
int
RoutingProtocol::DELAY (NeighborTuple const &tuple){
  return (tuple->prg*0.005*0.008);
}
      ...
      for (std::set<int>::iterator it = rs.begin (); it != rs.end (); it++)
        {
          ...
          for (std::vector<const NeighborTuple *>::iterator it2 = reachability[r].begin ();
               it2 != reachability[r].end (); it2++)
            {
              ...
              int progress=(20-sqrt(r*r+100))/(20);
              nb_tuple->prg=progress;
              if (progress>max_progress || max == NULL || nb_tuple->willingness > max->willingness)
                {
                  max = nb_tuple;
                  max_r = r; //max Euclidean range of a particular transmitter
                  max_progress=progress; //max progress achieved by selecting a particular neighbor
                }
